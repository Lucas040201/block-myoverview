...
const getWithEnrolmentEndDate = async courses => {
    ...
    return courses.map(course => {
        ...
        if(!currentEndDate || !currentEndDate.timeEnd) return course;
        const formatedTimeEnd = new Date(currentEndDate.timeEnd * 1000);
        const currentTime = new Date();
        const daysToEnd = Math.floor((formatedTimeEnd - currentTime) / (1000 * 60 * 60 * 24));
        if(daysToEnd < 100 && daysToEnd > 30) {
            expirationColor = 'yellow';
        } else if(daysToEnd < 31) {
            expirationColor = 'red';
        }
 ...
 * @return {promise} jQuery promise resolved after rendering is complete.
 */
const renderCourses = async (root, coursesData) => {
    ...
    if (!coursesData) {
        ...
        if (coursesData.courses.length) {
            return getWithEnrolmentEndDate(coursesData.courses).then(courses => {
